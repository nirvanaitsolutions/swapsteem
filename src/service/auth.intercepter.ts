...
export class AuthInterceptor implements HttpInterceptor {
  ...
  constructor(private auth: SteemconnectAuthService, private cookieService: CookieService, ) { }
  ...
  intercept(req: HttpRequest<any>, next: HttpHandler) {
    ...
    if (req.headers.get('No-Auth') == "True") {
      ...
      console.log(req.url)
      const authReq = req.clone({
        headers: req.headers.set('Authorization',`Bearer ${this.cookieService.get('user_token')}` )
